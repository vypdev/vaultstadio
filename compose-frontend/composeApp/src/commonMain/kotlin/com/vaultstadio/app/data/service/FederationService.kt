...
import com.vaultstadio.app.data.mapper.toFederatedActivityList
import com.vaultstadio.app.data.mapper.toFederatedShareList
import com.vaultstadio.app.data.mapper.toIdentityList
import com.vaultstadio.app.data.mapper.toInstanceList
...
class FederationService(private val api: FederationApi) {
    ...
    suspend fun getInstance(
        domain: String,
    ): ApiResult<FederatedInstance> = api.getInstance(domain).map { it.toDomain() }
    suspend fun blockInstance(instanceId: String): ApiResult<Unit> = api.blockInstance(instanceId)
    suspend fun removeInstance(instanceId: String): ApiResult<Unit> = api.removeInstance(instanceId)
    ...
    suspend fun getOutgoingShares(): ApiResult<List<FederatedShare>> = api.getOutgoingShares().map {
        it.toFederatedShareList()
    }
    ...
    suspend fun acceptShare(shareId: String): ApiResult<Unit> = api.acceptShare(shareId)
    suspend fun declineShare(shareId: String): ApiResult<Unit> = api.declineShare(shareId)
    suspend fun revokeShare(shareId: String): ApiResult<Unit> = api.revokeShare(shareId)
    ...
    suspend fun getIdentities(): ApiResult<List<FederatedIdentity>> = api.getIdentities().map { it.toIdentityList() }
    suspend fun unlinkIdentity(identityId: String): ApiResult<Unit> = api.unlinkIdentity(identityId)
