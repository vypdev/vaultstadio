/**
 * VaultStadio Federation Screen - Shared Components
 */
...
/**
 * Status chip for displaying instance status.
 */
@Composable
...
/**
 * Row displaying a detail label and value.
 */
@Composable
...
/**
 * Empty state for when there are no federated instances.
 */
@Composable
...
internal fun StatusChipPreview() {
    VaultStadioPreview {
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            StatusChip(status = InstanceStatus.ONLINE)
            StatusChip(status = InstanceStatus.PENDING)
            StatusChip(status = InstanceStatus.BLOCKED)
            StatusChip(status = InstanceStatus.OFFLINE)
        }
    }
}
...
internal fun DetailRowPreview() {
    VaultStadioPreview {
        DetailRow(label = "Domain", value = "example.vaultstadio.com")
    }
}

@Preview
@Composable
internal fun EmptyFederationStatePreview() {
    VaultStadioPreview {
