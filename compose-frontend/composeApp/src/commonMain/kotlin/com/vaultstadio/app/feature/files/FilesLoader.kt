...
internal class FilesLoader(
    ...
    suspend fun loadItemsForMode(
        ...
        return when (mode) {
            ...
            MainComponent.FilesMode.RECENT, MainComponent.FilesMode.STARRED, MainComponent.FilesMode.TRASH -> {
                ...
                when (listResult) {
                    is ApiResult.Success -> LoadItemsResult.Success(
                    ...
                    is ApiResult.Error -> LoadItemsResult.Error(listResult.message)
                    is ApiResult.NetworkError -> LoadItemsResult.Error(listResult.message)
                }
            }
        }
    }
    ...
    private suspend fun loadFolderItems(
        ...
        return when (folderResult) {
            ...
            is ApiResult.Error -> LoadItemsResult.Error(folderResult.message)
            is ApiResult.NetworkError -> LoadItemsResult.Error(folderResult.message)
        }
    }
    ...
    suspend fun loadMoreFolderItems(
        ...
        return when (result) {
            is ApiResult.Success -> LoadMoreResult.Success(
                newItems = result.data.items,
                hasMore = result.data.hasMore,
            )
            is ApiResult.Error -> LoadMoreResult.Error(result.message)
            is ApiResult.NetworkError -> LoadMoreResult.Error(result.message)
        }
    }
