...
internal suspend fun copyItemInternal(
    ...
    val newStorageKey = if (item.type == ItemType.FILE && item.storageKey != null) {
        when (val result = storageBackend.copy(item.storageKey)) {
            is Either.Left -> return result
            is Either.Right -> result.value
        }
    } else {
...
internal suspend fun deleteItemInternal(
    ...
    if (item.type == ItemType.FILE && item.storageKey != null) {
        when (val result = storageBackend.delete(item.storageKey)) {
            is Either.Left -> return result
