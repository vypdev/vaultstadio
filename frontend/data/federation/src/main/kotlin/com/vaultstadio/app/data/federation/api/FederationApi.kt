...
package com.vaultstadio.app.data.federation.api
...
import com.vaultstadio.app.data.federation.dto.RequestFederationRequestDTO
...
class FederationApi(client: HttpClient) : BaseApi(client) {
    ...
    suspend fun requestFederation(targetDomain: String, message: String?): ApiResult<FederatedInstanceDTO> =
        post("/api/v1/federation/instances/request", RequestFederationRequestDTO(targetDomain, message))
    ...
    suspend fun getInstance(domain: String): ApiResult<FederatedInstanceDTO> =
        get("/api/v1/federation/instances/$domain")

    suspend fun blockInstance(instanceId: String): ApiResult<Unit> =
        postNoBody("/api/v1/federation/instances/$instanceId/block")

    suspend fun removeInstance(instanceId: String): ApiResult<Unit> =
        delete("/api/v1/federation/instances/$instanceId")

    suspend fun createShare(request: CreateFederatedShareRequestDTO): ApiResult<FederatedShareDTO> =
        post("/api/v1/federation/shares", request)

    suspend fun getOutgoingShares(): ApiResult<List<FederatedShareDTO>> =
    ...
    suspend fun acceptShare(shareId: String): ApiResult<Unit> =
        postNoBody("/api/v1/federation/shares/$shareId/accept")

    suspend fun declineShare(shareId: String): ApiResult<Unit> =
        postNoBody("/api/v1/federation/shares/$shareId/decline")

    suspend fun revokeShare(shareId: String): ApiResult<Unit> =
        postNoBody("/api/v1/federation/shares/$shareId/revoke")

    suspend fun linkIdentity(request: LinkIdentityRequestDTO): ApiResult<FederatedIdentityDTO> =
        post("/api/v1/federation/identities", request)

    suspend fun getIdentities(): ApiResult<List<FederatedIdentityDTO>> =
    ...
    suspend fun getActivities(
        ...
        since: Instant?,
        limit: Int,
    ): ApiResult<List<FederatedActivityDTO>> {
