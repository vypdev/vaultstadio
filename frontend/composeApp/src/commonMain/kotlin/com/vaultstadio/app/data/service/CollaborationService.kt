...
import com.vaultstadio.app.data.mapper.toCommentList
import com.vaultstadio.app.data.mapper.toDomain
import com.vaultstadio.app.data.mapper.toParticipantList
import com.vaultstadio.app.data.mapper.toPresenceList
...
class CollaborationService(private val api: CollaborationApi) {
    suspend fun joinSession(
        itemId: String,
    ): ApiResult<CollaborationSession> = api.joinSession(itemId).map { it.toDomain() }
    suspend fun leaveSession(sessionId: String): ApiResult<Unit> = api.leaveSession(sessionId)
    suspend fun getSession(
        sessionId: String,
    ): ApiResult<CollaborationSession> = api.getSession(sessionId).map { it.toDomain() }
    ...
    suspend fun getDocumentState(
        itemId: String,
    ): ApiResult<DocumentState> = api.getDocumentState(itemId).map { it.toDomain() }
    ...
    suspend fun resolveComment(
        itemId: String,
        commentId: String,
    ): ApiResult<Unit> = api.resolveComment(itemId, commentId)
    suspend fun deleteComment(itemId: String, commentId: String): ApiResult<Unit> = api.deleteComment(itemId, commentId)
    ...
    suspend fun getUserPresence(
        userIds: List<String>,
    ): ApiResult<List<UserPresence>> = api.getUserPresence(userIds).map { it.toPresenceList() }
