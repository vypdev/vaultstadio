...
**Applied (continuation 2026-02-22):** **Backend core (share):** **ShareServiceTest** (DeactivateShareTests) extended with `should propagate when shareRepository update returns Left`. **Backend api (config):** **AppConfigTest** (DbConfigTests) extended with `should support runMigrations true`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionCopyLink**. —
**Applied (continuation 2026-02-22):** **Backend core (share):** **ShareServiceTest** extended with **DeleteShareTests** – `should propagate when shareRepository delete returns Left`; **GetSharesTests** – `should propagate when storageItemRepository findById returns Left for getSharesByItem`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionCopyAndActionMove**. —
**Applied (continuation 2026-02-22):** **Backend core (storage):** **StorageServiceTest** extended with **CopyItemTests** – `should fail with ValidationException when destination is not a folder`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionClose**. —
**Applied (continuation 2026-02-22):** **Backend core (storage):** **StorageServiceTest** extended with **MoveItemTests** – `should fail with ValidationException when destination is not a folder`. **Backend api (dto):** **ApiResponseTest** extended with `ApiError with empty details map`. —
**Applied (continuation 2026-02-22):** **Backend infrastructure (persistence):** **CollaborationContentApplyTest** extended with `applyOperationToContent Retain with empty content returns empty string`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionRename**. —
**Applied (continuation 2026-02-22):** **Backend core (storage):** **StorageServiceTest** (DownloadFileTests) extended with `should fail with ValidationException when item is a folder`. —
**Applied (continuation 2026-02-22):** **Backend core (storage):** **StorageServiceTest** extended with **DownloadFileTests** – `should fail with ValidationException when file has no storage key`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionDelete**. —
**Applied (continuation 2026-02-22):** **Backend infrastructure (storage):** **LocalStorageBackendTest** extended: `retrieve should return error for non-existent key` and `getSize should return error for non-existent key` now assert Left value is StorageBackendException. —
**Applied (continuation 2026-02-22):** **Backend core (share):** **ShareServiceTest** extended with `should propagate when storageItemRepository findById returns Left` (createShare error path when repository returns DatabaseException). —
**Applied (continuation 2026-02-22):** **Backend plugins (fulltext-search):** **FullTextSearchPluginTest** extended with `should have exact plugin id`, `should have exact plugin name`, `should support json and markdown`. **Backend core (version):** **FileVersionServiceTest** extended with `getVersionCount should propagate when repository returns Left`, `getTotalVersionSize should propagate when repository returns Left`. —
**Applied (continuation 2026-02-22):** **Backend plugins (image-metadata):** **ImageMetadataPluginTest** extended with `should have exact plugin name`, `should support heic heif and bmp`. **Backend plugins (video-metadata):** **VideoMetadataPluginTest** extended with `should have exact plugin id`, `should have exact plugin name`, `should support matroska and mpeg`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveNavSettings**. —
**Applied (continuation 2026-02-22):** **Backend infrastructure (persistence):** **CollaborationContentApplyTest** extended with `Delete with length zero leaves content unchanged`, `Insert with negative position clamped to start`. **Backend plugins (ai-classification):** **AIClassificationPluginTest** extended with `should have exact plugin id`, `should support gif and webp and bmp`. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** (CreateShareRequestTest) extended with `holds itemId and expirationDays only`. **Frontend composeApp:** **FederationModelsTest** (InstanceStatusTest) extended with **instanceStatus_allNames**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** (CreateFolderRequestTest) extended with `holds name with special characters`. **Frontend composeApp:** **StorageEnumsTest** (SortFieldTest) extended with **sortField_allNames**. —
**Applied (continuation 2026-02-22):** **Backend api:** **AppConfigTest** (StorageConfigTests) extended with `StorageConfig with S3 type holds s3 config`. **Frontend composeApp:** **AdminModelsTest** (AdminUserTest) extended with **adminUser_withAvatarUrlAndLastLoginAt**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** (RenameRequestTest) extended with `holds name with folder`. **Frontend composeApp:** **StorageModelsTest** (PaginatedResponseTest) extended with **paginatedResponse_emptyItems**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** (MoveRequestTest) extended with `holds destinationId with default newName null`. **Frontend composeApp:** **ActivityModelsTest** (ActivityTest) extended with **activity_withEmptyDetails**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** (PluginInfoResponseTest) extended with description and author assertions. **Frontend composeApp:** **ShareModelsTest** (ShareLinkTest) extended with **shareLink_withSharedWithUsers**. —
**Applied (continuation 2026-02-22):** **Backend core:** **StorageEventTest** extended with `FileEvent Downloaded without share uses defaults`. **Backend infrastructure:** **S3StorageBackendTest** (S3StorageConfigTests) extended with `should support bucket and region only`. **Frontend composeApp:** **MetadataModelsTest** extended with **metadataSearchResult_withEmptyValue**. —
**Applied (continuation 2026-02-22):** **Backend infrastructure:** **LocalStorageBackendTest** extended with `delete should succeed for non-existent key`, `constructor creates base directory when it does not exist`. **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveActionSave**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ErrorHandlingTest** (StatusCodeMappingTests) extended with `204 No Content should be created correctly`. **Frontend composeApp:** **VersionApiModelsTest** extended with **testFileVersionHistoryWithEmptyVersions**. —
**Applied (continuation 2026-02-22):** **Backend api:** **ThumbnailCacheTest** extended with ThumbnailCacheKey default version, ThumbnailCacheStats construction, CachedThumbnail equals false for null and other type. **Frontend composeApp:** **FederationApiModelsTest** extended with **testSharePermissionDeleteAndShare**. —
**Applied (continuation 2026-02-22):** **Backend api:** **UploadSessionManagerTest** (CreateAndGetTests) extended with `getSessionForUser returns null for missing upload id`. **Frontend composeApp:** **FormattingTest** extended with **truncateText_emptyString_returnsEmpty**, **truncateText_maxLengthThree_returnsEllipsisOnly**. —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **SyncModelsTest** extended with **SyncChangeTest** (construction, default optional nulls), **ConflictResolutionTest** (5 entries, names). **Backend api:** **AppConfigTest** (SecurityConfigTests) extended with `should support optional federation keys**. —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **SyncModelsTest** extended with **ConflictTypeTest** (6 entries, names), **ChangeTypeTest** (8 entries, names). **Backend api:** **AppConfigTest** (S3ConfigTests) extended with `should support usePathStyle false for AWS S3**. —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **SyncModelsTest** added (new file) – **SyncDeviceTest** (construction, default lastSyncAt null), **DeviceTypeTest** (8 entries, names). **Backend api:** **AppConfigTest** (DbConfigTests) extended with `should support runMigrations false`. —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **ShareModelsTest** added (new file) – **ShareLinkTest** (construction, defaults for createdBy and sharedWithUsers). **ActivityModelsTest** added (new file) – **ActivityTest** (construction, with null optionals). **Backend api:** **ApiResponseTest** extended with `ApiResponse success with default null data`. —
**Applied (continuation 2026-02-22):** **Frontend i18n (core/resources):** **StringsTest** extended with **loadSavedLanguage_withValidCode_setsLanguage**, **loadSavedLanguage_withNull_keepsCurrent**, **loadSavedLanguage_withUnknownCode_fallsBackToEnglish**. **Backend api:** **AppConfigTest** (HttpServerConfigTests) extended with `should support development flag`. —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **FederationModelsTest** extended with **FederationCapabilityTest** (6 entries, names), **FederatedIdentityTest** (construction, federatedId format), **FederatedActivityTest** (construction), **FederatedActivityTypeTest** (8 entries, names). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **MetadataModelsTest** added (new file) – **MetadataSearchResultTest** (construction). **FederationModelsTest** added (new file) – **FederatedInstanceTest** (construction, isOnline true/false), **InstanceStatusTest** (PENDING, ONLINE, OFFLINE, BLOCKED, REMOVED; names). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **AdminModelsTest** added (new file) – **AdminUserTest** (construction, usagePercentage when quota set, usagePercentage when quota null). **Backend api:** **ApiResponseTest** extended with **AdminUserResponseTest** (construction, with null optional fields). —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** extended with **PaginatedResponseTest** (api DTO construction, empty items and hasMore false). **Frontend composeApp:** **AdminEnumsTest** added (new file) – **UserStatusTest** (ACTIVE, INACTIVE, SUSPENDED; 3 entries, names). —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** extended with **StorageItemResponseTest** (construction and default metadata, with metadata), **UserResponseTest** (construction, with avatarUrl). **Frontend i18n (core/resources):** **StringsTest** extended with **allLanguages_haveAppName** (all 7 languages have non-empty appName). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **StorageEnumsTest** extended with **ItemTypeTest** (FILE, FOLDER, names), **VisibilityTest** (PRIVATE, SHARED, PUBLIC; 3 entries, names). **Backend api:** **ApiResponseTest** extended with **QuotaResponseTest** (construction, with null quotaBytes and remainingBytes), **ActivityResponseTest** (construction, with null optional fields). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **StorageModelsTest** extended with **BreadcrumbTest** (construction, with null id), **PaginatedResponseTest** (construction, hasMore false on last page). **Backend api:** **ApiResponseTest** extended with **ShareResponseTest** (construction and default sharedWithUsers, with sharedWithUsers list). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **StorageModelsTest** extended with **FolderUploadFileTest** (construction, equals uses only name and relativePath, equals false when name or path differs), **FolderUploadResultTest** (construction and defaults, withErrors), **FolderUploadErrorTest** (construction). **Backend api:** **AppConfigTest** (PluginsConfigTests) extended with `should support autoLoad false`. —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with **SystemEvent.PluginInstalled** – publish event and verify activityRepository.create with PLUGIN_INSTALLED, pluginId and pluginVersion in details. **Frontend composeApp:** **StorageModelsTest** extended with **ChunkedUploadInitTest** (construction), **ChunkedUploadStatusTest** (construction, isComplete true). —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with **SystemEvent.PluginUninstalled** – publish event and verify activityRepository.create with PLUGIN_UNINSTALLED and pluginId in details. **Frontend composeApp:** **StorageModelsTest** added (new file) – **BatchResultTest** (construction and defaults, withErrors), **BatchErrorTest** (construction). —
**Applied (continuation 2026-02-22):** **Frontend composeApp:** **StorageEnumsTest** added (new file) – **ViewModeTest** (GRID, LIST, names), **SortFieldTest** (NAME, SIZE, CREATED_AT, UPDATED_AT, TYPE; 5 entries), **SortOrderTest** (ASC, DESC, names). Covers domain.storage.model enums for coverage in other areas (plan: “Frontend – domain.model”). —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** extended with LoginResponseTest, RefreshResponseTest. **Frontend i18n:** **StringsTest** extended with allLanguages_haveErrorGeneric. —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with UserEvent.LoggedOut test. **Backend api:** **ApiResponseTest** extended with PluginConfigResponseTest. —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with ShareEvent.Deleted test. **Backend api:** **ApiResponseTest** extended with UpdateProfileRequestTest, ChangePasswordRequestTest. —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with FolderEvent.Moved test. **Backend api:** **ApiResponseTest** extended with RegisterRequestTest, LoginRequestTest, RefreshRequestTest. —
**Applied (continuation 2026-02-22):** **Backend api:** **ApiResponseTest** extended with CreateFolderRequestTest, RenameRequestTest, MoveRequestTest, CopyRequestTest. **Frontend i18n:** **StringsTest** extended with allLanguages_haveNavTrashAndActionCancel. —
**Applied (continuation 2026-02-22):** **Backend core/activity:** **ActivityLoggerTest** extended with FileEvent.Restored and FileEvent.Copied tests. **Frontend composeApp:** **VersionApiModelsTest** extended with FileVersionHistory and FileVersion.isRestore tests. —
