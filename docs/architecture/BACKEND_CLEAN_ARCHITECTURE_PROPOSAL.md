...
**Post Phase 2/3 (done)**:
- **Core tests** were moved to the corresponding `core:common`, `core:auth`, `core:storage`, etc. test source sets; imports updated to use `domain:common.exception`, `domain:auth`, `domain:storage`, etc. where applicable. `StorageExceptionTest` remains in `:core` with imports from `com.vaultstadio.domain.common.exception`. Model/event tests (e.g. `StorageEventTest`, `StorageItemTest`, `FileVersionTest`, `SyncTest`) remain in `:core` with domain imports where needed.
- **Quality**: FQN in `core/domain/event/StorageEvent.kt` fixed; `./scripts/check-no-fqn.sh` passes for backend. If `infrastructure` or `plugins-api` test compile fails (e.g. unresolved `User`, `UserQuery`), add the missing `domain:auth` (or other) test dependency and fix imports there.
