# Plugin Runtime and Extensibility

- Status: As-built baseline
- Date: 2026-09-21
- Catalog capability ID: `plugin-runtime-and-extensibility`
- Last verified: `603d84a2f88297c9e7fd1b7143df3980fb21e24d` (2026-09-20 UTC)
- Owners: VaultStadio maintainers
- Scope: Define plugin discovery, lifecycle, API, endpoint exposure, configuration, isolation, permissions, and failure containment.
...
Plugins extend metadata, search, video/image handling, and AI capabilities. This baseline defines extension ownership and the trust boundary between core application and plugin code.
...
Plugin architecture is described as a product principle, but runtime lifecycle, API compatibility, endpoint permissions, and isolation guarantees are not one contract.

### 2.2 Current behavior

The repository has `plugins-api`, plugin modules, `PluginManager`, plugin routes, application plugin use cases, and plugin tests.

### 2.3 Evidence

- `backend/plugins-api`
- `backend/plugins`
- `backend/application/plugin`
...
1. Define plugin lifecycle and compatibility.
2. Bound plugin permissions and resource use.
3. Separate plugin API contracts from core policy.
4. Make plugin failures isolated and observable.

### 4.2 Non-goals

1. No plugin receives implicit unrestricted access.
2. No runtime registry hides compile-time capability ownership without a deliberate decision.
3. Plugin endpoint exposure is not automatic authorization.

### 4.3 Fixed product and safety invariants

1. Plugin identity/version/capabilities are validated before enablement.
2. Disabled or failed plugins cannot mutate core state.
3. Plugin configuration is schema-validated and scoped.
4. Plugin API compatibility is explicit.

## 5. Current versus proposed product journey

| Stage | Current/observed | Contract to preserve or implement | User/operator effect |
|---|---|---|---|
| Entry | Operator discovers plugin | Validate identity, input, and configuration before mutation | Invalid work is rejected early |
| Operation | validates metadata/configuration | Apply one bounded semantic use case | Progress and ownership are explicit |
| Outcome | operator disables/upgrades/reconciles. | Return success, partial, or failure with retained state | Next action is inspectable |

Text equivalent: Operator discovers plugin -> validates metadata/configuration -> enables -> plugin registers capabilities -> user invokes feature -> plugin succeeds/fails boundedly -> operator disables/upgrades/reconciles..
...
| `enabled` | Capabilities registered | user/operator | use/configure/disable |
| `failed` | Initialization or call failure | operator | diagnose/disable/retry |
| `incompatible` | API/version mismatch | operator | upgrade/reject |

## 7. User-facing configuration

Plugin enablement, configuration schema, allowed capabilities, resource/time limits, endpoint exposure, and version compatibility are explicit. Unknown fields fail or are safely ignored only by documented policy.
...
|---|---|---|
| Domain/pure policy | Plugin Runtime and Extensibility invariants and provider-independent decisions | Ktor, Compose, database, SDK, process, or credentials |
...
Plugin API defines semantic capabilities. Application policies invoke ports. Plugin adapters implement extension behavior. Composition owns registration. Core domain does not import individual plugins.

Plugin manifest/schema, lifecycle events, endpoint registration, failure mapping, and compatibility policy are versioned and validated.
...
Plugin startup/call failure cannot bring unrelated core capabilities down. The system reports disabled/failed state and retains core data.
...
Plugin code is untrusted relative to core unless explicitly trusted; protect filesystem, network, secrets, code execution, endpoint and data access.
...
Record plugin ID/version/capability/state/duration/bounded failure; never plugin secrets or user content.
...
Minimum risk-derived budget: **Minimum 26 cases: manifest/schema, lifecycle, enable/disable, incompatible version, endpoint auth, malformed config, plugin failure isolation, resource limits, concurrency, and upgrade/recovery.**
...
1. Given an incompatible plugin manifest, when enablement is requested, then it is rejected before registration.
2. Given a plugin call failure, when a user invokes the feature, then core storage/auth remains available and the plugin enters a diagnosable failed state.
3. Given a disabled plugin, when its endpoint is called, then it is not exposed as active.
...
Inventory plugin API -> define manifest/trust contract -> add lifecycle tests -> validate endpoint/permission boundary -> publish operator and developer docs.
